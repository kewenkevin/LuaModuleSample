...
namespace ND.Managers.ResourceMgr.Framework
{
    ...
    public abstract class GameFrameworkSerializer<T>
    {
        ...
        private readonly Dictionary<byte, TryGetHeaderCallback> m_TryGetHeaderCallbacks;
        ...
        public GameFrameworkSerializer()
        {
            ...
            m_TryGetHeaderCallbacks = new Dictionary<byte, TryGetHeaderCallback>();
        ...
        public delegate bool TryGetHeaderCallback(Stream stream, out T headers);
        ...
        public void RegisterTryGetHeaderCallback(byte version, TryGetHeaderCallback callback)
        {
            if (callback == null)
            {
                throw new GameFrameworkException("Try get header callback is invalid.");
            }

            m_TryGetHeaderCallbacks[version] = callback;
        }
        ...
        /// <param name="value">指定键的值。</param>
        /// <returns>是否从指定流获取指定键的值成功。</returns>
        public bool TryGetHeader(Stream stream, out T headers)
        {
            headers = default(T);
            byte[] header = GetHeader();
            byte header0 = (byte)stream.ReadByte();
            ...
            byte version = (byte)stream.ReadByte();
            TryGetHeaderCallback callback = null;
            if (!m_TryGetHeaderCallbacks.TryGetValue(version, out callback))
            {
                return false;
            }

            return callback(stream, out headers);
        }
