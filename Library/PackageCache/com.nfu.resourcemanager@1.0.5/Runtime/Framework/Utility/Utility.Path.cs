        ...
        public static class Path
        {
            ...
            public static bool RemoveEmptyDirectory(string directoryName)
            {
                ...
                try
                {
                    ...
                    if (File.Exists(directoryName + ".meta"))
                    {
                        File.Delete(directoryName + ".meta");
                    }
                    return true;
                }
                catch
            ...
            public static bool DirectoryStartsWith(string path, string startDir)
            {
                if (path == null || startDir == null)
                    return false;
                path = path.ToLower().Trim().Replace('\\', '/');
                startDir = startDir.ToLower().Trim().Replace('\\', '/');

                if (!startDir.EndsWith("/"))
                    startDir += "/";
                return path.StartsWith(startDir);
            }
