...
using LOG=ND.Managers.ResourceMgr.Runtime.Log;
    ...
    public sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed class VersionListProcessor
        {
            ...
            public CheckVersionListResult CheckVersionList(int latestInternalResourceVersion, string latestApplicableGameVersion)
            {
                ...
                {
                    LOG.Info($"Check Resource Version NeedUpdate, local {RemoteVersionListFileName} not exists");
                ...
                try
                {
                    ...
                    if (!m_ResourceManager.m_UpdatableVersionListSerializer.TryGetHeader(fileStream, out var header))
                    {
                        LOG.Info($"Check Resource Version NeedUpdate, read local {RemoteVersionListFileName} header fail");
                        return CheckVersionListResult.NeedUpdate;
                    }

                    if (latestApplicableGameVersion != null && header.ApplicableGameVersion != latestApplicableGameVersion)
                    {
                        LOG.Info($"Check Resource Version NeedUpdate, ApplicableGameVersion changed  local: {latestApplicableGameVersion}, remote: {header.ApplicableGameVersion}");
                        return CheckVersionListResult.NeedUpdate;
                    }

                    internalResourceVersion = header.InternalResourceVersion;
                }
                catch
                {
                    LOG.Info($"Check Resource Version NeedUpdate, local {RemoteVersionListFileName} read fail");
                    return CheckVersionListResult.NeedUpdate;
                }
                finally
                {
                ...
                if (m_ResourceManager.UpdateCheckMode == ResourceUpdateMode.IgnoreVersion)
                {
                    if (internalResourceVersion != latestInternalResourceVersion)
                    {
                        LOG.Info($"Check Resource Version NeedUpdate, Mode: {m_ResourceManager.UpdateCheckMode}, InternalResourceVersion changed  local: {internalResourceVersion}, remote: {latestInternalResourceVersion}");
                        return CheckVersionListResult.NeedUpdate;
                    }
                }
                else
                ...
                LOG.Info($"Check Resource Version Updated, Mode: {m_ResourceManager.UpdateCheckMode}, InternalResourceVersion  local: {internalResourceVersion}, remote: {latestInternalResourceVersion}");
            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                using (FileStream fileStream = new FileStream(e.DownloadPath, FileMode.Open, FileAccess.ReadWrite))
                {
                    ...
                    try
                    {
                        ...
                        if (!Utility.ZipUtil.Decompress(fileStream, m_ResourceManager.m_DecompressCachedStream))
                        {
                            fileStream.Close();
