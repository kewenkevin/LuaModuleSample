...
namespace ND.Managers.ResourceMgr.Framework.Resource
{
    public sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        public sealed partial class ResourceLoader
        {
            ...
            //private readonly Dictionary<AssetObject, int> m_AssetDependencyCount;
            //private readonly Dictionary<ResourceObject, int> m_ResourceDependencyCount;
            //private readonly Dictionary<object, AssetObject> m_AssetToAssetObjectMap;
            private readonly Dictionary<string, AssetObject> m_SceneToAssetMap;
            ...
            /// 检查循环依赖资源
            /// </summary>
            private HashSet<string> checkCircleDependAssets = new HashSet<string>();
            /// <summary>
            /// 引用计数去重
            /// </summary>
            private HashSet<AssetObject> refCountDistinct = new HashSet<AssetObject>();

            public IObjectPool<AssetObject> AssetPool => m_AssetPool;
            public IObjectPool<ResourceObject> ResourcePool => m_ResourcePool;
            private LoadResourceAgent agent;
            ...
            public ResourceLoader(ResourceManager resourceManager)
            {
                ...
                //m_AssetDependencyCount = new Dictionary<AssetObject, int>();
                //m_ResourceDependencyCount = new Dictionary<ResourceObject, int>();
                //m_AssetToAssetObjectMap = new Dictionary<object, AssetObject>();
                m_SceneToAssetMap = new Dictionary<string, AssetObject>(StringComparer.Ordinal);
            ...
            public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData, bool rapidly = false)
            {
                assetName = FixName(assetName);
                ResourceInfo resourceInfo;

                AssetObject assetObject;
                //计算所有依赖引用计数
                assetObject = CreateOrSpawnAssetObject(assetName, assetType);

                if (assetObject == null)
                {
                    string[] dependencyAssetNames;
                    CheckAsset(ref assetName, out resourceInfo, out dependencyAssetNames);
                ...
                resourceInfo = assetObject.Resource.ResourceInfo;
                ...
                LoadAssetTask mainTask = LoadAssetTask.Create(assetObject, assetType, priority, loadAssetCallbacks, userData, rapidly);

                foreach (var dependencyAsset in assetObject.DependencyAssets)
                {
                    if (!LoadDependencyAsset(dependencyAsset, priority, mainTask, userData, rapidly))
                    {
                        string errorMessage = Utility.Text.Format("Can not load dependency asset '{0}' when load asset '{1}'.", dependencyAsset.AssetName, assetName);
                        if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                        {
            ...
            /// 同步加载
            /// </summary>
            public Object LoadAssetImmediate(string assetName, Type assetType = null)
            {
                assetName = FixName(assetName);
                //计算引用计数
                var assetObject = CreateOrSpawnAssetObject(assetName, assetType);
                if (!assetObject.IsLoaded)
                {
                    LoadAssetObjectImmediate(assetObject, false);
                }

                assetObject.GetRealAsset();

                return assetObject.GetAsset(assetType) as Object;
            }

            /// <summary>
            /// 同步加载资源，依赖已经计算过，只需处理资源加载
            /// </summary>
            private void LoadAssetObjectImmediate(AssetObject assetObject, bool depend)
            {
                if (assetObject.IsLoaded)
                    return;
                ...
                {
                    //已在异步加载任务中
                    if (assetObject.IsLoadAsync)
                    {
                        Debug.LogWarning("Asset already async loading");
                    }
                    else
                    {
                        //已经在同步加载中
                ...
                foreach (var dependencyAsset in assetObject.DependencyAssets)
                {
                    if (dependencyAsset.IsLoaded)
                        continue;
                    LoadAssetObjectImmediate(dependencyAsset, true);
                }


                AssetBundle bundle;
                List<AssetObject> dependencyAssets = new List<AssetObject>();

                ResourceObject resourceObject = assetObject.Resource;

                if (!resourceObject.IsResourceLoaded)
                {
                    if (resourceObject.IsLoading && resourceObject.IsLoadAsync)
                    {
                        Debug.LogWarning($"call sync load resource, already async loading. {resourceObject.Name}");
                    }

                    resourceObject.OnLoading(false);

                    var resourceInfo = resourceObject.ResourceInfo;
                    Log.Info($"Load bundle '{resourceObject.Name}'");
                    byte[] bytes;
                    string fullPath = null;
                    var mgr = ResourceEntry.GetComponent<ResourceComponent>().ResourceManager;
                    fullPath = Utility.Path.GetRegularPath(Path.Combine(resourceInfo.StorageInReadOnly ? mgr.ReadOnlyPath : mgr.ReadWritePath, resourceInfo.UseFileSystem ? resourceInfo.FileSystemName : resourceInfo.ResourceName.FullName));
                    ...
                    if (!isFile && ResourcesAndroidUtility.IsObb)
                    {
                        isFile = false;
                    }
                    ...
                    if (isFile)
                    {
                        ...
                        {
                            bundle = AssetBundle.LoadFromFile(fullPath.Substring(7));
                        }
                        else
                        {
                    ...
                    {
                        //从内存读取
                        var stream = m_ResourceManager.m_FileSystemManager.CreateFileSystemStream(fullPath, FileSystemAccess.Read);

                        if (stream == null)
                        {
                            Debug.LogError($"load asset bundle file error. bundle: { resourceInfo.ResourceName.Name}");
                            return;
                        }
                    ...
                    if (bundle == null)
                    {
                        Debug.LogError("asset bundle null: " + resourceInfo.ResourceName.Name);
                        return;
                    }

                    resourceObject.OnLoaded(bundle);
                }
            ...
            /// <summary>
            /// 对资源引用进行计数，如果没有资源则创建并计数
            /// </summary>
            AssetObject CreateOrSpawnAssetObject(string assetName, Type assetType)
            {
                checkCircleDependAssets.Clear();
                refCountDistinct.Clear();

                return CreateOrSpawnAssetObject(assetName, assetType, checkCircleDependAssets, refCountDistinct);
            }

            AssetObject CreateOrSpawnAssetObject(string assetName, Type assetType, HashSet<string> checkCircleDependAssets, HashSet<AssetObject> spawnAssetRefCount)
            {
                ResourceInfo resourceInfo;
                string[] dependencyAssetNames;

                if (!CheckAsset(ref assetName, out resourceInfo, out dependencyAssetNames))
                {
                    return null;
                }

                string key = GetAssetFullName(assetName, assetType);

                AssetObject assetObject = AssetPool.Peek(key);
                //检查循环依赖
                if (checkCircleDependAssets.Contains(key))
                    return assetObject;
                checkCircleDependAssets.Add(key);


                if (assetObject == null)
                {
                    ResourceObject resourceObject;
                    resourceObject = CreateOrSpawnResourceObject(resourceInfo);

                    assetObject = AssetObject.Create(key, assetName, assetType, null, resourceObject, agent.ResourceHelper, this);
                    AssetPool.Register(assetObject, true);
                    spawnAssetRefCount.Add(assetObject);
                    AssetObject[] dependencyAssets = new AssetObject[dependencyAssetNames.Length];

                    for (int i = 0; i < dependencyAssetNames.Length; i++)
                    {

                        var dependencyAsset = CreateOrSpawnAssetObject(dependencyAssetNames[i], null, checkCircleDependAssets, spawnAssetRefCount);
                        if (dependencyAsset == null)
                            continue;
                        dependencyAssets[i] = dependencyAsset;
                        if (dependencyAsset != null && dependencyAsset.Resource != null)
                        {
                            resourceObject.AddDependencyResource(dependencyAsset.Resource);
                        }
                        else
                        {
                            throw new GameFrameworkException("Can not find dependency resource. " + dependencyAsset.Name);
                        }
                    }

                    assetObject.SetDependencyAssets(dependencyAssets.Where(o => o != null));
                }
                else
                {
                    if (!spawnAssetRefCount.Contains(assetObject))
                    {
                        AssetPool.Spawn(key);
                        spawnAssetRefCount.Add(assetObject);
                    }
            ...
            ResourceObject CreateOrSpawnResourceObject(ResourceInfo resourceInfo)
            {
                ResourceObject resourceObject = m_ResourcePool.Spawn(resourceInfo.ResourceName.Name);
                if (resourceObject == null)
                {
                    resourceObject = ResourceObject.Create(resourceInfo.ResourceName.Name, agent.ResourceHelper, this);
                    //注册 bundle
                    m_ResourcePool.Register(resourceObject, true);
                    resourceObject.ResourceInfo = resourceInfo;
                }
            ...
            public void UnloadAsset(string assetName, Type assetType)
            {
                assetName = FixName(assetName);
                ...
                var key = GetAssetFullName(assetName, assetType);
                m_AssetPool.Unspawn(key);
            }
            ...
            /// <returns></returns>
            public AssetObject PeekAssetObject(object asset)
            {
                return m_AssetPool.Peek(asset);
            }
            ...
            /// <param name="userData">用户自定义数据。</param>
            public void LoadScene(string sceneAssetName, int priority, LoadSceneCallbacks loadSceneCallbacks, object userData, bool rapidly = false)
            {
                sceneAssetName = FixName(sceneAssetName);
                AssetObject assetObject;
                ResourceInfo resourceInfo;

                //计算所有依赖引用计数
                assetObject = CreateOrSpawnAssetObject(sceneAssetName, null);
                ...
                LoadSceneTask mainTask = LoadSceneTask.Create(assetObject, priority, loadSceneCallbacks, userData, rapidly);
                foreach (var dependencyAsset in assetObject.DependencyAssets)
                {
                    if (!LoadDependencyAsset(dependencyAsset, priority, mainTask, userData))
                    {
                        string errorMessage = Utility.Text.Format("Can not load dependency asset '{0}' when load scene '{1}'.", dependencyAsset.AssetName, sceneAssetName);
                        if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                        {
            ...
            {
                binaryAssetName = FixName(binaryAssetName);
                ResourceInfo resourceInfo = GetResourceInfo(binaryAssetName);
                if (resourceInfo == null)
                {
                    Debug.LogError($"resourceInfo null. asset: {binaryAssetName}");
                    return null;
                }

                if (!resourceInfo.Ready)
                {
                    Debug.LogError($"resource Ready false. asset: {binaryAssetName}");
                    return null;
                }

                if (!resourceInfo.IsLoadFromBinary)
                {
                    Debug.LogError($"resource IsLoadFromBinary false . asset: {binaryAssetName}");
                    return null;
                }

                if (resourceInfo.UseFileSystem)
                {
                    Debug.LogError($"resource UseFileSystem true. asset: {binaryAssetName}");
                    return null;
                }
            ...
            private bool LoadDependencyAsset(AssetObject assetObject, int priority, LoadResourceTaskBase mainTask, object userData, bool rapidly = false)
            {
                if (mainTask == null)
                {
                ...
                {
                    // #region 优先级自动提前
                    // if (priority > TaskBase.DefaultPriority)
                    // {
                    //     var waitTasks = m_TaskPool.WaitingTaskes;
                    //     var pNode = waitTasks.First;
                    //     LinkedListNode<LoadResourceTaskBase> target = pNode;
                    //     while (pNode != null)
                    //     {
                    //         if (assetObject == pNode.Value.AssetObject)
                    //         {
                    //             if (priority > pNode.Value.Priority)
                    //                 target = pNode;
                    //             break;
                    //         }
                    //
                    ...
                    //         var needMoveTask = target.Value;
                    //         waitTasks.Remove(target);
                    //
                    //         needMoveTask.Priority = priority;
                    //         if (rapidly) needMoveTask.IsRapidly = true;
                    //
                    //         if (priority >= int.MaxValue)
                    //         {
                    //             var current = waitTasks.First;
                    //             while (current != null)
                    //             {
                    //                 if (needMoveTask.Priority > current.Value.Priority)
                    //                 {
                    //                     break;
                    //                 }
                    //                 
                    ...
                    //             if (current != null)
                    //             {
                    //                 waitTasks.AddBefore(current, needMoveTask);
                    //             }
                    //             else
                    //             {
                    //                 waitTasks.AddFirst(needMoveTask);
                    //             }
                    //         }
                    //         else
                    //         {
                    ...
                    //             if (current != null)
                    //             {
                    //                 waitTasks.AddAfter(current, needMoveTask);
                    //             }
                    //             else
                    //             {
                    //                 waitTasks.AddFirst(needMoveTask);
                ...
                ResourceInfo resourceInfo = assetObject.Resource.ResourceInfo;
                ...
                LoadDependencyAssetTask parentTask = null;

                //依赖资源只需要资源包加载完成             
                if (assetObject.Resource.IsLoaded)
                {
                    assetObject.OnLoading(false);
                    assetObject.SetDelayLoad();
                }
                ...
                {
                    parentTask = LoadDependencyAssetTask.Create(assetObject, priority, mainTask, userData, rapidly);
                }
                foreach (var dependencyAsset in assetObject.DependencyAssets)
                {
                    if (!LoadDependencyAsset(dependencyAsset, priority, parentTask ?? mainTask, userData, rapidly))
                    {
                        return false;
                    }
                }
                if (parentTask != null)
                    m_TaskPool.AddTask(parentTask);
                if (!resourceInfo.Ready)
                {
                    m_ResourceManager.UpdateResource(resourceInfo.ResourceName);
                }
            ...
            internal bool CheckAsset(ref string assetName, out ResourceInfo resourceInfo, out string[] dependencyAssetNames)
            {
                resourceInfo = null;
                ...
                for (int i = 0; i < dependencyAssetNames.Length; i++)
                {
                    dependencyAssetNames[i] = m_ResourceManager.TranslateToVariant(dependencyAssetNames[i]);
                }
            ...
            /// 配合ResourceBuilderController打包脚本一起修改
            /// </summary>
            /// <returns></returns>
            private static string FixName(string assetName)
            {
                if (string.IsNullOrEmpty(assetName))
                    return assetName;
