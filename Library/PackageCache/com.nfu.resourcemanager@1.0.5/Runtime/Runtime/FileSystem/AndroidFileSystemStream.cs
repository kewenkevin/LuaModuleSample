...
namespace ND.Managers.ResourceMgr.Runtime
{
    ...
    public sealed class AndroidFileSystemStream : FileSystemStream
    {
        ...
        public AndroidFileSystemStream(string fullPath, FileSystemAccess access, bool createNew)
        {
            ...
            {
                //android obb文件
                m_FileStream = ResourcesAndroidUtility.GetStreamingAssetsInputStream(fileName);
                Debug.Log("Obb Stream " + (m_FileStream != null));
            }
            else
            {
        ...
        public override int Read(byte[] buffer, int startIndex, int length)
        {
            ...
            //这里由于类型转换问题 需要用Buffer对内存块直接拷贝
            // Array.Copy(result, 0, buffer, startIndex, bytesRead);
            Buffer.BlockCopy(result, 0, buffer, startIndex, bytesRead);
            return bytesRead;
        }
