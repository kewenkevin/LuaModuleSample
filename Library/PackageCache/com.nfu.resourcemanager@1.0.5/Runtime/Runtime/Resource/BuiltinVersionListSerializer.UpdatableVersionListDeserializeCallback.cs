    ...
    public static partial class BuiltinVersionListSerializer
    {
        ...
        public static UpdatableVersionList UpdatableVersionListDeserializeCallback_V0(Stream stream)
        {
            using (BinaryReader binaryReader = new BinaryReader(stream, Encoding.UTF8))
            {
                ...
                return new UpdatableVersionList(applicableGameVersion, internalResourceVersion, assets, resources, null, resourceGroups,variantPrefixUrl);
            }
        }
        ...
        public static UpdatableVersionList UpdatableVersionListDeserializeCallback_V2(Stream stream)
        {
            using (BinaryReader binaryReader = new BinaryReader(stream, Encoding.UTF8))
            {
                ...
                for (int i = 0; i < resourceCount; i++)
                {
                    ...
                    if (Utility.Path.DirectoryStartsWith(name, ResourceSettings.RawAssetsPath))
                        extension = null;
                    resources[i] = new UpdatableVersionList.Resource(name, variant, extension, loadType, length, hashCode, compressedLength, compressedHashCode, assetIndexes);
                }
                ...
                return new UpdatableVersionList(applicableGameVersion, internalResourceVersion, assets, resources, fileSystems, resourceGroups,variantPrefixUrl);
            }
        }
    }
}
