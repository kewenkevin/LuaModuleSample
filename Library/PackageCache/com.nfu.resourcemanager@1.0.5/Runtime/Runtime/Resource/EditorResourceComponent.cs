...
namespace ND.Managers.ResourceMgr.Runtime
{
    ...
    public sealed class EditorResourceComponent : MonoBehaviour, IResourceManager
    {
        ...
        [Tooltip("Editor下是否缓存资源，即相同asset其LoadAssetAtPath只执行一次")]
        [SerializeField]
        private bool m_EnableCachedAssets = true;

        [Tooltip("单帧加载资源数量限制,用来模拟资源加载Task数量")]
        [SerializeField]
        private int m_LoadAssetCountPerFrame = 1;

        [Tooltip("Editor下模拟异步资源加载的随机时间最小值")]
        [SerializeField]
        private float m_MinLoadAssetRandomDelaySeconds = 0f;

        [Tooltip("Editor下模拟异步资源加载的随机时间最大值")]
        [SerializeField]
        private float m_MaxLoadAssetRandomDelaySeconds = 0f;


        [Tooltip("Editor下模拟异步场景加载的随机时间最小值")]
        [SerializeField]
        private float m_MinLoadSceneRandomDelaySeconds = 0f;

        [Tooltip("Editor下模拟异步场景加载的随机时间最大值")]
        [SerializeField]
        private float m_MaxLoadSceneRandomDelaySeconds = 0f;

        private string m_ReadOnlyPath = null;
        private string m_ReadWritePath = null;
        private Dictionary<KeyValuePair<Type, string>, UnityEngine.Object> m_CachedAssets = null;
        ...
        /// 更新检测模式
        /// </summary>
        public ResourceUpdateMode UpdateCheckMode
        {
            get;
            set;
        }
        ...
        public IObjectPool<ResourceManager.ResourceLoader.AssetObject> AssetPool { get; set; }
        public IObjectPool<ResourceManager.ResourceLoader.ResourceObject> ResourcePool { get; set; }
        ...
        public bool Initializated { get; set; } = true;
...
#pragma warning disable 0067, 0414
        ...
        /// 资源应用成功事件。
        /// </summary>
        public event EventHandler<ND.Managers.ResourceMgr.Framework.Resource.ResourceApplySuccessEventArgs> ResourceApplySuccess = null;

        /// <summary>
        /// 资源应用失败事件。
        /// </summary>
        public event EventHandler<ND.Managers.ResourceMgr.Framework.Resource.ResourceApplyFailureEventArgs> ResourceApplyFailure = null;

        /// <summary>
        /// 资源更新开始事件。
        /// </summary>
        public event EventHandler<ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateStartEventArgs> ResourceUpdateStart = null;

        /// <summary>
        /// 资源更新改变事件。
        /// </summary>
        public event EventHandler<ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateChangedEventArgs> ResourceUpdateChanged = null;

        /// <summary>
        /// 资源更新成功事件。
        /// </summary>
        public event EventHandler<ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateSuccessEventArgs> ResourceUpdateSuccess = null;

        /// <summary>
        /// 资源更新失败事件。
        /// </summary>
        public event EventHandler<ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateFailureEventArgs> ResourceUpdateFailure = null;
...
#pragma warning restore 0067, 0414
        ...
        private void Awake()
        {
            ...
            m_CachedAssets = new Dictionary<KeyValuePair<Type, string>, UnityEngine.Object>();
        ...
        private void Update()
        {
            if (m_LoadAssetInfos.Count > 0)
            {
                ...
                while (current != null && count < m_LoadAssetCountPerFrame)
                {
                    ...
                    {
                        UnityEngine.Object asset = LoadAssetImmediate(loadAssetInfo.AssetName, loadAssetInfo.AssetType);
            ...
            if (m_LoadSceneInfos.Count > 0)
            {
                ...
                while (current != null)
                {
                    ...
                    float elapseSeconds = (float)(DateTime.Now - loadSceneInfo.StartTime).TotalSeconds;
                    if (loadSceneInfo.AsyncOperation == null)
                    {
                        if (elapseSeconds >= loadSceneInfo.DelaySeconds)
                        {
#if UNITY_5_5_OR_NEWER  
                            #if UNITY_EDITOR
                            //AsyncOperation asyncOperation =
                            //    SceneManager.LoadSceneAsync(loadSceneInfo.SceneAssetName, LoadSceneMode.Additive);
                            AsyncOperation asyncOperation = UnityEditor.SceneManagement.EditorSceneManager.LoadSceneAsyncInPlayMode(
                                loadSceneInfo.SceneAssetName, new LoadSceneParameters() {loadSceneMode = LoadSceneMode.Additive});
                            #else
                                AsyncOperation asyncOperation =
                                    SceneManager.LoadSceneAsync(loadSceneInfo.SceneAssetName, LoadSceneMode.Additive);
                            #endif
#else
                            AsyncOperation asyncOperation =
 SceneManager.LoadSceneAsync(SceneComponent.GetSceneName(loadSceneInfo.SceneAssetName), LoadSceneMode.Additive);
#endif
                            ...
                            if (asyncOperation == null)
                            {
                                ...
                                {
                                    loadSceneInfo.LoadSceneCallbacks.LoadSceneFailureCallback(
                                        loadSceneInfo.SceneAssetName, LoadResourceStatus.AssetError,
                            ...
                            {
                                m_LoadSceneInfos.AddLast(new LoadSceneInfo(asyncOperation, loadSceneInfo.SceneAssetName, loadSceneInfo.Priority, loadSceneInfo.StartTime, loadSceneInfo.DelaySeconds, loadSceneInfo.LoadSceneCallbacks, loadSceneInfo.UserData));
                                LinkedListNode<LoadSceneInfo> next = current.Next;
                                m_LoadSceneInfos.Remove(loadSceneInfo);
                                current = next;
                            }
                        ...
                        else
                        {
                            try
                            {
                                ...
                                {
                                    loadSceneInfo.LoadSceneCallbacks.LoadSceneUpdateCallback(loadSceneInfo.SceneAssetName, (elapseSeconds / loadSceneInfo.DelaySeconds) * 0.5f, loadSceneInfo.UserData);
                                }
                            }
                            catch (Exception e)
                            {
                    ...
                    else
                    {
                        if (loadSceneInfo.AsyncOperation.isDone)
                        {
                            ...
                            try
                            {
                                ...
                                if (loadSceneInfo.AsyncOperation.allowSceneActivation)
                                {
                                    ...
                                    {
                                        loadSceneInfo.LoadSceneCallbacks.LoadSceneSuccessCallback(
                                          //  loadSceneInfo.SceneAssetName,
                                            (float)(DateTime.Now - loadSceneInfo.StartTime).TotalSeconds,
                        ...
                        else
                        {
                            try
                            {
                                if (loadSceneInfo.LoadSceneCallbacks.LoadSceneUpdateCallback != null)
                                {
                                    loadSceneInfo.LoadSceneCallbacks.LoadSceneUpdateCallback(
                                        loadSceneInfo.SceneAssetName, loadSceneInfo.AsyncOperation.progress,
                                        loadSceneInfo.UserData);
                                }
                            }
                            catch (Exception e)
        ...
        {
            Log.Warning("在Editor模拟模式下设置{0}变体无效，Editor模拟模式下的变体将使用Setting面板中的设置！", currentVariant);
        ...
        {
            return TranslateToEditorVariant(assetName);
        }

        public string GetResourceFullName(string assetName)
        {
            return TranslateToEditorVariant(assetName);
        }
        public string GetResourceFileName(string assetName)
        {
            return TranslateToEditorVariant(assetName);
        }

        public string GetResourcePath(string assetName)
        {
            return TranslateToEditorVariant(assetName);
        }
        ...
        public CheckVersionListResult CheckVersionList(int latestInternalResourceVersion, string latestApplicableGameVersion)
        {
        ...
        public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            ...
            if (!assetName.StartsWith("Assets/", StringComparison.InvariantCultureIgnoreCase))
            {
                if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                {
        ...
        #region Rapid Load Asset & Bundle
        ...
        /// <param name="assetName">要加载资源的名称。</param>
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        public void LoadAssetRapid(string assetName, LoadAssetCallbacks loadAssetCallbacks)
        {
            LoadAssetRapid(assetName, null, DefaultPriority, loadAssetCallbacks, null);
        }
        ...
        /// <param name="assetType">要加载资源的类型。</param>
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        public void LoadAssetRapid(string assetName, Type assetType, LoadAssetCallbacks loadAssetCallbacks)
        {
            LoadAssetRapid(assetName, assetType, DefaultPriority, loadAssetCallbacks, null);
        }
        ...
        /// <param name="priority">加载资源的优先级。</param>
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        public void LoadAssetRapid(string assetName, int priority, LoadAssetCallbacks loadAssetCallbacks)
        {
            LoadAssetRapid(assetName, null, priority, loadAssetCallbacks, null);
        }
        ...
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void LoadAssetRapid(string assetName, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            LoadAssetRapid(assetName, null, DefaultPriority, loadAssetCallbacks, null);
        }
        ...
        /// <param name="priority">加载资源的优先级。</param>
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        public void LoadAssetRapid(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks)
        {
            LoadAssetRapid(assetName, assetType, priority, loadAssetCallbacks, null);
        }
        ...
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void LoadAssetRapid(string assetName, Type assetType, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            LoadAssetRapid(assetName, assetType, DefaultPriority, loadAssetCallbacks, userData);
        }
        ...
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void LoadAssetRapid(string assetName, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            LoadAssetRapid(assetName, null, priority, loadAssetCallbacks, userData);
        }
        ...
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void LoadAssetRapid(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
        ...
        public void LoadScene(string sceneAssetName, int priority, LoadSceneCallbacks loadSceneCallbacks, object userData)
        {
            ...
            if (!sceneAssetName.StartsWith("Assets/", StringComparison.InvariantCultureIgnoreCase) || !sceneAssetName.EndsWith(".unity", StringComparison.Ordinal))
            {
                if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                {
            ...
            {
                loadSceneCallbacks.LoadSceneSuccessCallback( 0, userData);
            }
            
           // m_LoadSceneInfos.AddLast(new LoadSceneInfo(null, sceneAssetName, priority, DateTime.Now, m_MinLoadSceneRandomDelaySeconds + (float)Utility.Random.GetRandomDouble() * (m_MaxLoadSceneRandomDelaySeconds - m_MinLoadSceneRandomDelaySeconds), loadSceneCallbacks, userData));
        }
...
#if UNITY_5_5_OR_NEWER
#if UNITY_EDITOR
            // AsyncOperation asyncOperation = UnityEditor.SceneManagement.EditorSceneManager.UnloadSceneAsync(
            //     sceneAssetName);    
            #else
        ...
        {
            if (!HasFile(binaryAssetName, null))
            {
                return null;
            }
        ...
        public void LoadBinary(string binaryAssetName, LoadBinaryCallbacks loadBinaryCallbacks, object userData)
        {
            ...
            if (!binaryAssetName.StartsWith("Assets/", StringComparison.InvariantCultureIgnoreCase))
            {
                if (loadBinaryCallbacks.LoadBinaryFailureCallback != null)
                {
        ...
        {
#if UNITY_EDITOR
            var asset = AssetDatabase.LoadAssetAtPath<TextAsset>(binaryAssetName);
            return asset.bytes;
#endif
        ...
        public string[] GetResourceAllAssets(string resourceName)
        {
            throw new NotSupportedException("GetResourceAllAssets");
        }

        public object[] GetPackageSubAsset(object asset)
        {
#if UNITY_EDITOR
            if (asset == null)
            {
            ...
            var path = UnityEditor.AssetDatabase.GetAssetPath(asset as Texture2D);
            var subObjects = UnityEditor.AssetDatabase.LoadAllAssetsAtPath(path).Where(o => asset != o).ToArray();
            return subObjects;
#else
            throw new NotSupportedException("GetPackageSubAsset");
            return null;
#endif

        }

        private bool HasFile(string assetName, Type assetType = null)
        {
            if (string.IsNullOrEmpty(assetName))
            {
                return false;
        ...
        private bool HasCachedAsset(string assetName, Type assetType)
        {
            ...
            return m_CachedAssets.ContainsKey(GetAssetKey(assetName, assetType));
        }
        /// <summary>
        /// 支持 SubAsset
        /// </summary>
        KeyValuePair<Type, string> GetAssetKey(string assetName, Type assetType)
        {
            return new KeyValuePair<Type, string>(assetType, assetName);
        }
        private UnityEngine.Object GetCachedAsset(string assetName, Type assetType)
        {
            ...
            if (m_CachedAssets.TryGetValue(GetAssetKey(assetName, assetType), out asset))
            {
                return asset;
            }
        ...
        {
            var key = GetAssetKey(assetName, assetType);
            UnityEngine.Object asset = GetCachedAsset(assetName, assetType);
            if (asset == null)
            {
#if UNITY_EDITOR
                //设置变体
                ...
                {
                    assetName = TranslateToEditorVariant(assetName);
                }
                ...
                {
                    asset = AssetDatabase.LoadMainAssetAtPath(assetName);    
                }
                ...
                {
                    Debug.LogError("not found asset name:  " + key.Value + " > " + assetName);
                }
#endif
            }
            return asset;
        ...
        public static string TranslateToEditorVariant(string assetName)
        {
            return string.Format("{0}{1}/{2}", ResourceUserSettings.VariantPrefixUrl, ResourceUserSettings.Variant, assetName);
        }
        ...
        private struct LoadSceneInfo
        {
            ...
            public LoadSceneInfo(AsyncOperation asyncOperation, string sceneAssetName, int priority, DateTime startTime, float delaySeconds, LoadSceneCallbacks loadSceneCallbacks, object userData)
            {
