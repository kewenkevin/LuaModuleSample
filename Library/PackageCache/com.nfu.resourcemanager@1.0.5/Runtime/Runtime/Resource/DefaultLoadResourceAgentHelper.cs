...
namespace ND.Managers.ResourceMgr.Runtime
{
    ...
    public class DefaultLoadResourceAgentHelper : LoadResourceAgentHelperBase, IDisposable
    {
        ...
        private bool m_AssetBundleLoadAll = false;
        ...
        public override void ReadFile(string fullPath , bool rapidly)
        {
            ...
            { 
                OnSyncFile(AssetBundle.LoadFromFile(fullPath));
            }
        ...
        public override void ReadFile(IFileSystem fileSystem, string name , bool rapidly)
        {
            ...
            { 
                OnSyncFile(AssetBundle.LoadFromFile(fileSystem.FullPath, 0u, (ulong) fileInfo.Offset));
            }
        ...
        /// </summary>
        /// <param name="fullPath">要加载资源的完整路径名。</param>
        public override void ReadBytes(string fullPath,bool rapidly)
        {
        ...
        /// <param name="fileSystem">要加载资源的文件系统。</param>
        /// <param name="name">要加载资源的名称。</param>
        public override void ReadBytes(IFileSystem fileSystem, string name , bool rapidly)
        {
            if (m_LoadResourceAgentHelperReadBytesCompleteEventHandler == null || m_LoadResourceAgentHelperUpdateEventHandler == null || m_LoadResourceAgentHelperErrorEventHandler == null)
            {
                Log.Fatal("Load resource agent helper handler is invalid.");
            ...
            LoadResourceAgentHelperReadBytesCompleteEventArgs loadResourceAgentHelperReadBytesCompleteEventArgs = LoadResourceAgentHelperReadBytesCompleteEventArgs.Create(bytes,rapidly);
        ...
        public override void ParseBytes(byte[] bytes, bool rapidly)
        {
            ...
            {
                OnSyncBytes(AssetBundle.LoadFromMemory(bytes));
            }
        }
        ...
        public override void LoadAsset(object resource, string resourceName, string assetName, Type assetType, bool isScene, bool dependency, AssetObject assetObject , bool rapidly)
        {
            ...
            if (isScene)
            {
                ...
                LoadResourceAgentHelperLoadCompleteEventArgs loadResourceAgentHelperLoadCompleteEventArgs = LoadResourceAgentHelperLoadCompleteEventArgs.Create(new SceneAsset(), null);
            ...
            else
            {
                ...
                if (dependency)
                {
                    if (assetObject.AssetName.EndsWith(".spriteatlas"))
                    {
                        //需要注册图集
                    ...
                    OnLoadAsset(assetObject.Target, assetObject.subAssets, false);
                }
                else
                {
                    //加载场景
                    if (assetBundle.isStreamedSceneAssetBundle)
                    {
                        ...
                        OnLoadAsset(empty, new object[] { empty }, false);
                    }
                    else if (assetType != null)
                    {
                        if (!rapidly)
                        {
                            m_AssetBundleLoadAll = false;
                            m_AssetBundleRequest = assetBundle.LoadAssetWithSubAssetsAsync(assetName, assetType);
                        }
                        else
                        {
                            var assets = assetBundle.LoadAssetWithSubAssets(assetName, assetType);

                            if (assets != null && assets.Length> 0 )
                            {
                                OnLoadAsset(assets[0], assets, true);
                            }
                            else
                            {
                                OnLoadAsset(null, null, true);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            m_AssetBundleLoadAll = false;
                            m_AssetBundleRequest = assetBundle.LoadAssetWithSubAssetsAsync(assetName);
                        }
                        else
                        {
        ...
        private void UpdateUnityWebRequest()
        {
            if (m_UnityWebRequest != null)
            {
                if (m_UnityWebRequest.isDone)
                {
                    ...
                    {
                        LoadResourceAgentHelperReadBytesCompleteEventArgs loadResourceAgentHelperReadBytesCompleteEventArgs = LoadResourceAgentHelperReadBytesCompleteEventArgs.Create(m_UnityWebRequest.downloadHandler.data,false);
                        m_LoadResourceAgentHelperReadBytesCompleteEventHandler(this, loadResourceAgentHelperReadBytesCompleteEventArgs);
                        ReferencePool.Release(loadResourceAgentHelperReadBytesCompleteEventArgs);
                        m_UnityWebRequest.Dispose();
                        m_UnityWebRequest = null;
        ...
        private void OnSyncFile(AssetBundle assetBundle)
        {
        ...
        private void OnSyncBytes(AssetBundle assetBundle)
        {
        ...
        private void UpdateAssetBundleRequest()
        {
            if (m_AssetBundleRequest != null)
            {
                if (m_AssetBundleRequest.isDone)
                {
                    ...
                    {
                        OnLoadAsset(m_AssetBundleRequest.asset, m_AssetBundleRequest.allAssets, true);
                    }
                    else
                    {
                        OnLoadAsset(null, null, true);
        ...
        void OnLoadAsset(object asset, object[] subassets, bool rawSubAssets)
        {
            if (asset != null)
            {
                LoadResourceAgentHelperLoadCompleteEventArgs loadResourceAgentHelperLoadCompleteEventArgs = LoadResourceAgentHelperLoadCompleteEventArgs.Create(asset, subassets, m_AssetBundleLoadAll, rawSubAssets);
                m_LoadResourceAgentHelperLoadCompleteEventHandler(this, loadResourceAgentHelperLoadCompleteEventArgs);
                ReferencePool.Release(loadResourceAgentHelperLoadCompleteEventArgs);
                m_AssetName = null;
                m_LastProgress = 0f;
