...
namespace ND.Managers.ResourceMgr.Runtime
{
    ...
    [AddComponentMenu("ResourceMgr/Resource")]
    [DefaultExecutionOrder(-10)]
    public sealed class ResourceComponent : GameFrameworkComponent
    {
        private const int DefaultPriority = 0;
        private const int DefaultRapidPriority = int.MaxValue;
        private const int OneMegaBytes = 1024 * 1024;

        private IResourceManager m_ResourceManager = null;
        private ResGCReleaseAgent m_ResGCReleaseAgent = null;
        ...
        [SerializeField]
        private ResourceUpdateMode m_UpdateCheckMode = ResourceUpdateMode.IgnoreVersion;
        ...
        [SerializeField]
        private string m_GUISpriteAtlasPrefixedUrl = string.Empty;

        public IResourceManager ResourceManager => m_ResourceManager;


        public ResGCReleaseAgent ResGCReleaseAgent => m_ResGCReleaseAgent;
        ...
        public ResourceUpdateMode UpdateCheckMode
        {
            get
            {
                return m_ResourceManager.UpdateCheckMode;
            }
            set
            {
                m_ResourceManager.UpdateCheckMode = m_UpdateCheckMode = value;
            }
        }
        ...
        /// 初始化完成
        /// </summary>
        public bool Initializated { get => ResourceManager != null && ResourceManager.Initializated; }

        /// <summary>
        /// 缓存的atlas请求回调
        /// </summary>
        public static Dictionary<string, System.Action<SpriteAtlas>> m_requestMap = new Dictionary<string, Action<SpriteAtlas>>();
        /// <summary>
        /// 缓存图集
        /// </summary>
        public static Dictionary<string, SpriteAtlas> cachedAtlas = new Dictionary<string, SpriteAtlas>();
        ...
        {
            Debug.Assert(atlas);
            Log.Debug("[RegisterSpriteAtlas] {0}", atlas.tag);
            cachedAtlas[atlas.tag] = atlas;
            if (m_requestMap.ContainsKey(atlas.tag))
            {
                Log.Debug("[RegisterSpriteAtlas Invoke] {0}", atlas.tag);
                m_requestMap[atlas.tag]?.Invoke(atlas);
                m_requestMap.Remove(atlas.tag);
            }
        }

        /// <summary>
        /// 这里需要一个规则, spriteatlas和自己包含的所有sprite打在一起，用spriteatlas的路径作为tag 这样spriteatlas不需要单独生命周期管理
        /// </summary>
        ...
        {
            SpriteAtlas atlas;
            if (cachedAtlas.TryGetValue(tag, out atlas) && atlas)
            {
                Log.Debug("[RequestAtlas Invoke] {0}", atlas.tag);
                callback(atlas);
                return;
            }

            if (!m_requestMap.ContainsKey(tag))
            {
                m_requestMap.Add(tag, callback);
            }
            
            string path = string.Format("{0}/{1}.spriteatlas",
                !string.IsNullOrEmpty(ResourceSettings.AtlasPath) ? ResourceSettings.AtlasPath :
                string.IsNullOrEmpty(m_GUISpriteAtlasPrefixedUrl) ? "Assets/ResourcesAssets/UI/GUI" : m_GUISpriteAtlasPrefixedUrl, tag);

            Log.Debug("[RequestAtlas Begin] {0}", path);
            LoadAssetRapid(path, new LoadAssetCallbacks((assetName, asset, duration, data) =>
            {
                Log.Debug("[RequestAtlas Success] {0}", path);
                ...
                //延迟一帧等底层使用 SpriteAtlas，否则会导致循环 RequestAtlas
                Delay(() =>
                {
            ...
            {
                Debug.LogWarningFormat("[RequestAtlas Error] {0} asset: {1}, status: {2}, msg: {3}", path, assetName, status, message);
            }));
        }
        private void Delay(Action callback)
        {
            StartCoroutine(_Delay(callback));
        }
        private IEnumerator _Delay(Action callback)
        {
            yield return null;
            callback();
        }
        ...
        private void Start()
        {
            ...
            BaseComponent baseComponent = ResourceEntry.GetComponent<BaseComponent>();
            ...
            m_ResGCReleaseAgent = new ResGCReleaseAgent();
            ...
            string path = System.IO.Path.Combine(Application.streamingAssetsPath, ResourceSettings.StreamingAssetsPath);
            Debug.Log("SetReadOnlyPath: " + path);
            m_ResourceManager.SetReadOnlyPath(path);
            ...
            {
                SetResourceMode(ResourceMode.Package);
            }
            else  if (Application.isEditor)
            {
                if (ResourceUserSettings.ResourceMode == ResourceMode.Unspecified)
                {
                    //默认 Streaming 加载
                    SetResourceMode(ResourceMode.Package);
                }
                ...
                {
                    //编辑器下优先使用菜单选择的
                    SetResourceMode(ResourceUserSettings.ResourceMode);
                }
            }
            else
            {
                //运行时使用预设 m_ResourceMode 设置值
                SetResourceMode(m_ResourceMode);
            }
            Debug.Log("Resouce mode: " + ResourceMode);
            ...
            if (ResourceMode == ResourceMode.Updatable || ResourceMode == ResourceMode.UpdatableWhilePlaying)
            {
                ...
                m_ResourceManager.UpdateCheckMode = m_UpdateCheckMode;
            }
        ...
        private void Update()
        {
            ...
            m_ResGCReleaseAgent?.UnloadReal();
        }
        ...
        private void SetResourceMode(ResourceMode resourceMode)
        {
            ...
            switch (resourceMode)
            {
                ...
                case ResourceMode.UpdatableWhilePlaying:
                    ...
                    m_ResourceManager.UpdatableVersionListSerializer.RegisterTryGetHeaderCallback(0, BuiltinVersionListSerializer.UpdatableVersionListTryGetHeaderCallback_V0);
                    m_ResourceManager.UpdatableVersionListSerializer.RegisterTryGetHeaderCallback(1, BuiltinVersionListSerializer.UpdatableVersionListTryGetHeaderCallback_V1_V2);
                    m_ResourceManager.UpdatableVersionListSerializer.RegisterTryGetHeaderCallback(2, BuiltinVersionListSerializer.UpdatableVersionListTryGetHeaderCallback_V1_V2);
        ...
        public CheckVersionListResult CheckVersionList(int latestInternalResourceVersion, string latestApplicableGameVersion = null)
        {
            return m_ResourceManager.CheckVersionList(latestInternalResourceVersion, latestApplicableGameVersion);
        }
        ...
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void LoadAsset(string assetName, System.Type assetType, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
        ...
        public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            ...
            if (!assetName.StartsWith("Assets/", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                loadAssetCallbacks.LoadAssetFailureCallback?.Invoke(assetName,LoadResourceStatus.NotExist,string.Format("Asset name '{0}' is invalid.", assetName), userData);
                return;
            }
        ...
        #region Rapid Load Asset & AssetBundle
        ...
        {
            LoadAssetRapid(assetName, null, DefaultRapidPriority, loadAssetCallbacks, null);
        }
        ...
        {
            LoadAssetRapid(assetName, assetType, DefaultRapidPriority, loadAssetCallbacks, null);
        }
        ...
        {
            LoadAssetRapid(assetName, null, DefaultRapidPriority, loadAssetCallbacks, userData);
        }
        ...
        /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void LoadAssetRapid(string assetName, System.Type assetType, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            LoadAssetRapid(assetName, assetType, DefaultRapidPriority, loadAssetCallbacks, userData);
        }
        ...
        public void LoadAssetRapid(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            ...
            m_ResourceManager.LoadAssetRapid(assetName, assetType, priority, loadAssetCallbacks, userData);
        }
        ...
        {
            return m_ResourceManager.LoadAssetImmediate(assetName, assetType);
        }
        ...
        public void UnloadAsset(string assetName, Type assetType = null)
        {
            m_ResourceManager.UnloadAsset(assetName, assetType);
        }
        /// <summary>
        /// 获取二进制资源的实际路径。
        /// </summary>
        ...
        private void OnResourceApplySuccess(object sender, ND.Managers.ResourceMgr.Framework.Resource.ResourceApplySuccessEventArgs e)
        {
            ResourceApplySuccess?.Invoke(this,ResourceApplySuccessEventArgs.Create(e));
        }

        private void OnResourceApplyFailure(object sender, ND.Managers.ResourceMgr.Framework.Resource.ResourceApplyFailureEventArgs e)
        {
            ResourceApplyFailure?.Invoke(this,ResourceApplyFailureEventArgs.Create(e));
        }

        private void OnResourceUpdateStart(object sender, ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateStartEventArgs e)
        {
            ResourceUpdateStart?.Invoke(this,ResourceUpdateStartEventArgs.Create(e));
        }

        private void OnResourceUpdateChanged(object sender, ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateChangedEventArgs e)
        { 
            ResourceUpdateChanged?.Invoke(this,ResourceUpdateChangedEventArgs.Create(e));
        }

        private void OnResourceUpdateSuccess(object sender, ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateSuccessEventArgs e)
        { 
            ResourceUpdateSuccess?.Invoke(this,ResourceUpdateSuccessEventArgs.Create(e));
        }

        private void OnResourceUpdateFailure(object sender, ND.Managers.ResourceMgr.Framework.Resource.ResourceUpdateFailureEventArgs e)
        {
            ResourceUpdateFailure?.Invoke(this,ResourceUpdateFailureEventArgs.Create(e));
        }
    }
}
