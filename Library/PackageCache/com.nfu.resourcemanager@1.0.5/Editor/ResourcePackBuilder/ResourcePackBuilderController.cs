...
namespace ND.Managers.ResourceMgr.Editor.ResourceTools
{
    public sealed class ResourcePackBuilderController
    {
        ...
        public string RemoteVersionListSearchPattern
        {
            get
            {
                var length = ResourceBuilderController.RemoteVersionListFileName.LastIndexOf('.');
                if (length == -1)
                {
                    return $"{ResourceBuilderController.RemoteVersionListFileName}.*";
                }
                    
                var fileName = ResourceBuilderController.RemoteVersionListFileName.Substring(0, length);
                var extName = ResourceBuilderController.RemoteVersionListFileName.Substring(length);
        ...
        public string SourcePathForDisplay
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/Full/*/{1}/", WorkingDirectory, EditorResourceSettings.GetPlatformName(Platform))).FullName);
            }
        }
        ...
        public string OutputPath
        {
            get
            {
                ...
                return Utility.Path.GetRegularPath(new DirectoryInfo(Utility.Text.Format("{0}/ResourcePack/{1}/", WorkingDirectory, EditorResourceSettings.GetPlatformName(Platform))).FullName);
            }
        }
        ...
        {
            if (Platform == Platform.FollowProject || !IsValidWorkingDirectory)
            {
            ...
            foreach (DirectoryInfo directoryInfo in sourceDirectoryInfo.GetDirectories())
            {
                ...
                FileInfo[] versionListFiles = platformDirectoryInfo.GetFiles(RemoteVersionListSearchPattern, SearchOption.TopDirectoryOnly);
        ...
        public bool BuildResourcePack(string sourceVersion, string targetVersion)
        {
            ...
            {
                DirectoryInfo sourceDirectoryInfo = new DirectoryInfo(Path.Combine(Path.Combine(SourcePath, sourceVersion), EditorResourceSettings.GetPlatformName(Platform)));
                FileInfo[] sourceVersionListFiles = sourceDirectoryInfo.GetFiles(RemoteVersionListSearchPattern, SearchOption.TopDirectoryOnly);
                byte[] sourceVersionListBytes = File.ReadAllBytes(sourceVersionListFiles[0].FullName);
                sourceVersionListBytes = Utility.ZipUtil.Decompress(sourceVersionListBytes);
            ...
            DirectoryInfo targetDirectoryInfo = new DirectoryInfo(Path.Combine(Path.Combine(SourcePath, targetVersion), EditorResourceSettings.GetPlatformName(Platform)));
            FileInfo[] targetVersionListFiles = targetDirectoryInfo.GetFiles(RemoteVersionListSearchPattern, SearchOption.TopDirectoryOnly);
            byte[] targetVersionListBytes = File.ReadAllBytes(targetVersionListFiles[0].FullName);
            targetVersionListBytes = Utility.ZipUtil.Decompress(targetVersionListBytes);
