...
namespace ND.Managers.ResourceMgr.Editor.ResourceTools
{
    ...
    internal sealed class ResourcePackBuilder : EditorWindow
    {
        ...
        [MenuItem(EditorUtilityx.AdvancedMenuPrefix + "资源打包编辑器", false, 44)]
        ...
        private void OnGUI()
        {
            EditorGUILayout.BeginVertical(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    EditorGUILayout.EndHorizontal();
                    if (m_Controller.Platform == Platform.FollowProject || !m_Controller.IsValidWorkingDirectory)
                    {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUI.BeginDisabledGroup(m_Controller.Platform == Platform.FollowProject || !m_Controller.IsValidWorkingDirectory || m_SourceVersionCount <= 0);
                    {
        ...
        private void OnBuildResourcePacksStarted(int count)
        {
            ...
            EditorUtilityx.DisplayProgressBar("Build Resource Packs", Utility.Text.Format("Build resource packs, {0} items to be built.", count.ToString()), 0f);
        }
        ...
        private void OnBuildResourcePackSuccess(int index, int count, string sourceVersion, string targetVersion)
        {
            ...
            EditorUtilityx.DisplayProgressBar("Build Resource Packs", Utility.Text.Format("Build resource packs, {0}/{1} completed.", (index + 1).ToString(), count.ToString()), (float)index / count);
        }
