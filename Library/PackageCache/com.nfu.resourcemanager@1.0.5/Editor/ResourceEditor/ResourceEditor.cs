...
namespace ND.Managers.ResourceMgr.Editor.ResourceTools
{
    ...
    {
        private ResourceEditorImplate m_ResourceImplate;
        private ResourceEditorController m_Controller => m_ResourceImplate.Controller;
        ...
        private HashSet<ResourceFolder> m_CachedSelectedResourceFolders = null;
        private HashSet<ResourceFolder> m_CachedUnselectedResourceFolders = null;
        ...
        [MenuItem(EditorUtilityx.AdvancedMenuPrefix + "资源编辑器", false, 42)]
        ...
        {
            m_ResourceImplate = new ResourceEditorImplate();
            ...
            m_CachedSelectedResourceFolders = new HashSet<ResourceFolder>();
            m_CachedUnselectedResourceFolders = new HashSet<ResourceFolder>();
            ...
            ResourceEditorImplate.Changed -= ResourceEditorImplate_Changed;
            ResourceEditorImplate.Changed += ResourceEditorImplate_Changed;
        }


        private void ResourceEditorImplate_Changed()
        {
            if (EditorUtility.DisplayDialog("Reload?", "New analysis result reload?","OK","Cancel"))
            {
                m_Reload = true;
            }
        }
        ...
        private void Load()
        {
            ...
            m_ResourceImplate.Load();
            m_ResourceImplate.Controller.ResourceCollection.Load();
            RefreshResourceTree();
        }
        ...
        private void OnGUI()
        {
            EditorGUILayout.BeginHorizontal(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(Utility.Text.Format("资源包列表 ({0})", m_Controller.ResourceCount.ToString()), EditorStyles.boldLabel);
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(Utility.Text.Format("资源包中内容 ({0})", m_CurrentResourceContentCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginHorizontal("box", GUILayout.Height(position.height - 52f));
                    {
                        DrawResourceView();
                    }
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField("工程资源列表", EditorStyles.boldLabel);
                    EditorGUILayout.BeginHorizontal("box", GUILayout.Height(position.height - 52f));
                    {
            ...
            if (isAutoAnalysis)
            {
                ...
                m_ResourceImplate.ClearAll();
                m_ResourceImplate.AutoAnalysis();

                if (m_Controller.AutoSave)
                {
                    SaveConfiguration();
                }
                RefreshResourceTree();
        ...
        private void DrawResourceFolder(ResourceFolder folder)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                bool select = IsSelectedResourceFolder(folder);
                if (select != EditorGUILayout.Toggle(select, GUILayout.Width(12f + 14f * folder.Depth)))
                {
                    select = !select;
                    SetSelectedResourceFolder(folder, select);
                }
                ...
                EditorGUILayout.LabelField(string.Empty, GUILayout.Width(30f), GUILayout.Height(18f));
#else
                GUI.DrawTexture(new Rect(32f + 14f * folder.Depth, 20f * m_CurrentResourceRowOnDraw + 1f, 16f, 16f), ResourceFolder.Icon);
                EditorGUILayout.LabelField(string.Empty, GUILayout.Width(26f + 14f * folder.Depth), GUILayout.Height(18f));
#endif
        ...
        private void DrawResourcesMenu_Normal()
        {
            ...
            EditorGUI.BeginDisabledGroup(m_SelectedResource == null && m_SelectedResources.Count == 0);
            {
        ...
        private void DrawResourcesMenu_Remove()
        {
            ...
            if (m_SelectedResource == null && m_SelectedResources.Count == 0)
            {
                m_MenuState = MenuState.Normal;
                return;
            }
            ...
            {
                GUILayout.Label(Utility.Text.Format("Remove Folder(s) ?"));

                if (GUILayout.Button("Yes", GUILayout.Width(50f)))
                {
                    EditorUtility.DisplayProgressBar("Remove Resource", "Processing...", 0f);
                    if (m_SelectedResource != null && m_SelectedResources.Contains(m_SelectedResource) == false)
                    {
                        RemoveResource(m_SelectedResource);
                    }
                    RemoveResource(m_SelectedResources);
                    EditorUtility.ClearProgressBar();
                    m_MenuState = MenuState.Normal;
                }
            ...
            else if (m_SelectedResource != null)
            {
        ...
        {
            if (GUILayout.Button("所有", GUILayout.Width(50f)) && m_SelectedResource != null)
            {
                Asset[] assets = m_Controller.GetAssets(m_SelectedResource.Name, m_SelectedResource.Variant);
                foreach (Asset asset in assets)
                {
        ...
        private void DrawSourceAssetsMenu()
        {
            ...
            EditorGUI.BeginDisabledGroup(selectedSourceAssets.Count <= 0);
            {
                if (GUILayout.Button(Utility.Text.Format("<<< {0}", selectedSourceAssets.Count.ToString()), GUILayout.Width(80f)))
                {
                    ...
                    EditorUtility.DisplayProgressBar("添加资源", "Complete processing...", 1f);
                    RefreshResourceTree();
                    EditorUtility.ClearProgressBar();
                    m_SelectedSourceAssets.Clear();
                    m_CachedSelectedSourceFolders.Clear();
            ...
            EditorGUI.EndDisabledGroup();
            bool hideAssignedSourceAssets = EditorGUILayout.ToggleLeft("隐藏已标记记录", m_HideAssignedSourceAssets, GUILayout.Width(100f));
            ...
            DrawAnalysisStrategySelector();

            if (GUILayout.Button("清除所有记录", GUILayout.ExpandWidth(false)))
            {
                m_ResourceImplate.ClearAll();
                RefreshResourceTree();
                if (m_Controller.AutoSave)
            ...
            if (GUILayout.Button("自动分析", GUILayout.ExpandWidth(false)))
            {
                isAutoAnalysis = true;
            }

            if (GUILayout.Button("清理临时", GUILayout.ExpandWidth(false)))
            {
            ...
            if (GUILayout.Button("保存修改", GUILayout.ExpandWidth(false)))
            {
            ...
            m_Controller.AutoSave = EditorGUILayout.ToggleLeft(new GUIContent("自动保存", "分析结果和清理自动保存"), m_Controller.AutoSave, GUILayout.ExpandWidth(false));
        ...
        private void DrawSourceAsset(SourceAsset sourceAsset)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.LabelField(asset != null ? m_ResourceImplate.GetResourceFullName(asset.Resource.Name, asset.Resource.Variant) : string.Empty, GUILayout.Width(position.width * 0.15f));
            }
            EditorGUILayout.EndHorizontal();
            m_CurrentSourceRowOnDraw++;
        }
        ...
        private void SaveConfiguration()
        {
            if (m_Controller.Save())
            {
                ...
                if (EditorResourceSettings.SetAssetBundleName && m_Controller.ResourceCount > 0)
                {
        ...
        private void AddResource(string name, string variant, bool refresh)
        {
            ...
            string fullName = m_ResourceImplate.GetResourceFullName(name, variant);
        ...
        private void RenameResource(Resource resource, string newName, string newVariant)
        {
            ...
            string newFullName = m_ResourceImplate.GetResourceFullName(newName, newVariant);
        ...
        private void RemoveResource(Resource resource)
        {
            ...
            if (m_Controller.RemoveResource(resource.Name, resource.Variant))
            {
                ChangeSelectedResource(null);
                RefreshResourceTree();
        ...
        private void RemoveResource(HashSet<Resource> resources)
        {
            ...
            m_CachedSelectedResourceFolders.Clear();
        }
        ...
        private string[] analysisStrategyNames { get => m_ResourceImplate.analysisStrategyNames; }
        private int m_analysisStrategySelectIndex
        {
            get => m_ResourceImplate.m_analysisStrategySelectIndex;
            set => m_ResourceImplate.m_analysisStrategySelectIndex = value;
        }

        private void DrawAnalysisStrategySelector()
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("分析策略", GUILayout.Width(50));
            int selectedIndex = EditorGUILayout.Popup(m_analysisStrategySelectIndex, analysisStrategyNames);
            if (selectedIndex != m_analysisStrategySelectIndex)
            {
                m_analysisStrategySelectIndex = selectedIndex;
                m_Controller.AnalysisHandlerName = analysisStrategyNames[selectedIndex];
            }
            EditorGUILayout.EndHorizontal();
        }
        ...
        private bool IsSelectedResource(Resource resource)
        {
            return m_SelectedResources.Contains(resource);
        }
        
        private void SetSelectedResourceItem(ResourceItem resourceItem, bool select)
        {
            if (select)
            {
                m_SelectedResources.Add(resourceItem.Resource);

                ResourceFolder folder = resourceItem.Folder;
                while (folder != null)
                {
                    m_CachedUnselectedResourceFolders.Remove(folder);
                    folder = folder.Folder;
                }
            }
            else
            {
                m_SelectedResources.Remove(resourceItem.Resource);

                ResourceFolder folder = resourceItem.Folder;
                while (folder != null)
                {
                    m_CachedSelectedResourceFolders.Remove(folder);
                    folder = folder.Folder;
                }
            }
        }
        
        private bool IsSelectedResourceFolder(ResourceFolder resourceFolder)
        {
            if (m_CachedSelectedResourceFolders.Contains(resourceFolder))
            {
                return true;
            }

            if (m_CachedUnselectedResourceFolders.Contains(resourceFolder))
            {
                return false;
            }

            foreach (ResourceItem resourceItem in resourceFolder.GetItems())
            {
                if (!IsSelectedResource(resourceItem.Resource))
                {
                    m_CachedUnselectedResourceFolders.Add(resourceFolder);
                    return false;
                }
            }

            foreach (ResourceFolder subResourceFolder in resourceFolder.GetFolders())
            {
                if (!IsSelectedResourceFolder(subResourceFolder))
                {
                    m_CachedUnselectedResourceFolders.Add(resourceFolder);
                    return false;
                }
        ...
        private void SetSelectedResourceFolder(ResourceFolder resourceFolder, bool select)
        {
            if (select)
            {
                m_CachedSelectedResourceFolders.Add(resourceFolder);
                m_CachedUnselectedResourceFolders.Remove(resourceFolder);
            ...
            {
                m_CachedSelectedResourceFolders.Remove(resourceFolder);
                m_CachedUnselectedResourceFolders.Add(resourceFolder);
            ...
            foreach (ResourceItem resourceItem in resourceFolder.GetItems())
            {
                SetSelectedResourceItem(resourceItem, select);
            }

            foreach (ResourceFolder subResourceFolder in resourceFolder.GetFolders())
            {
                SetSelectedResourceFolder(subResourceFolder, select);
            }
        }
        ...
        public void RefreshResourceTree()
        {
            ...
            foreach (Resource resource in resources)
            {
                ...
                ResourceEditor.ResourceFolder folder = m_ResourceRoot;
                for (int i = 0; i < splitedPath.Length - 1; i++)
                {
                    ResourceEditor.ResourceFolder subFolder = folder.GetFolder(splitedPath[i]);
