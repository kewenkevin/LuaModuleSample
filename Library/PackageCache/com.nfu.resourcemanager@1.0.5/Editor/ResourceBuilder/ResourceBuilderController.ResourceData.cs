...
namespace ND.Managers.ResourceMgr.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        private sealed class ResourceData
        {
            ...
            {
                string[] assetPaths = new string[m_AssetDatas.Count];
                for (int i = 0; i < m_AssetDatas.Count; i++)
                {
                    assetPaths[i] = m_AssetDatas[i].AssetPath;
                }
                return assetPaths;
            }
            ...
            public AssetData AddAssetData(string guid, string name, int length, int hashCode, string[] dependencyAssetNames)
            {
                AssetData assetData = new AssetData(guid, name, length, hashCode, dependencyAssetNames);
                m_AssetDatas.Add(assetData);
                return assetData;
            }
