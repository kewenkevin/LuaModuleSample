...
namespace ND.Managers.ResourceMgr.Editor.ResourceTools
{
    ...
    internal sealed class ResourceBuilder : EditorWindow
    {
        ...
        [MenuItem(EditorUtilityx.AdvancedMenuPrefix + "资源构建编辑器", false, 41)]
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.BeginVertical();
                    {
                        ...
                        EditorGUILayout.BeginHorizontal("box");
                        {
                            EditorGUILayout.BeginVertical();
                            {
                                DrawPlatform(Platform.FollowProject, "Follow project");
        ...
        {
            var performance = new PerformanceSample($"{GetType().Name}.{nameof(BuildResources)}");
        ...
        {
            if (platform == Platform.FollowProject)
            {
                if (EditorGUILayout.ToggleLeft(platformName, m_Controller.Platforms == platform))
                {
            ...
            {
                m_Controller.SelectPlatform(platform, EditorGUILayout.ToggleLeft(platformName, ((m_Controller.Platforms & platform) != 0)));
            }
        }

        private void OnLoadingResource(int index, int count)
        {
            EditorUtilityx.DisplayProgressBar("Loading Resources", Utility.Text.Format("Loading resources, {0}/{1} loaded.", index.ToString(), count.ToString()), (float)index / count);
        }

        private void OnLoadingAsset(int index, int count)
        {
            EditorUtilityx.DisplayProgressBar("Loading Assets", Utility.Text.Format("Loading assets, {0}/{1} loaded.", index.ToString(), count.ToString()), (float)index / count);
        }
        ...
        {
            EditorUtilityx.DisplayProgressBar("Analyzing Assets", Utility.Text.Format("Analyzing assets, {0}/{1} analyzed.", index.ToString(), count.ToString()), (float)index / count);
        }
