...
{
    [AddComponentMenu("NDUI/NDSlider", 33)]
    [ExecuteAlways]
    ...
    /// </remarks>
    public class NDSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        ...
        [SerializeField]
        private float m_TweenDuration = 0.135f; // Only used when tween is enabled
        ...
        public Text text { get { return m_Text; } set { if (SetPropertyUtility.SetClass(ref m_Text, value)) { UpdateVisuals(); } } }

        [Space]

        [SerializeField]
        private TextType m_TextType = TextType.Percent;
        ...
        private NDTweenRunner<IndFloatTween> m_ValueTweenRunner;

        protected NDSlider()
        {
            if (m_ValueTweenRunner == null)
                m_ValueTweenRunner = new NDTweenRunner<IndFloatTween>();
            m_ValueTweenRunner.Init(this);
        }
        ...
        protected void TweenValue(float newValue)
        {
            var tween = new IndFloatTween();
            tween.duration = tweenDuration;
            tween.startValue = m_Value;
            tween.targetValue = newValue;
            tween.AddOnChangedCallback((float val,bool finish)=>
            {
                m_Value = val;
                UpdateVisuals();
                if (finish)
            ...
            m_ValueTweenRunner.StartTween(tween);
        }
        ...
        private void UpdateVisuals()
        {
            ...
            if (m_Text != null)
            {
                if(m_TextType == TextType.Percent)
                    m_Text.text = string.Format("{0}%", Mathf.FloorToInt((reverseValue ? 1 - normalizedValue : normalizedValue) * 100));
                else
        ...
        void UpdateDrag(PointerEventData eventData, Camera cam)
        {
            ...
            if (clickRect != null && clickRect.rect.size[(int)axis] > 0)
            {
                ...
                if (!MultipleDisplayUtilities2.GetRelativeMousePositionForDrag(eventData, ref position))
                    return;
