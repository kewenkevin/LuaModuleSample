...
namespace ND.UI
{
    public class TextColorLineTreeItem : TransitionLineTreeItemBase
    {
        private Color _monitorCheckValue;

        private Color _previewRecordValue;

        private Text LineTarget
        {
            get { return Target.GetComponent<Text>(); }
        }

        public TextColorLineTreeItem() : base(LineTypeState.TextColor)
        {
            _canUseTween = true;
            _valueSize = 4;
        }
        ...
        public override void RefreshMonitorValue()
        {
            ...
            if (frameItem != null)
            {
                _monitorCheckValue = frameItem.GetColorValue();
            }
            else
            {
