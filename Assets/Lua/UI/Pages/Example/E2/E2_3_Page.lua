...
---@class E2_3_Page : UIPageBase
local E2_3_Page = class("E2_3_Page", UIPageBase)


---region 页面数据组织
function E2_3_Page:data()
    local E2_3_PageData = 
    {
        msgText = "",
        inputPasswordText = "",
    }
    return E2_3_PageData
end


---methods 构造绑定的方法列表， 用来完成子模块emit事件响应注册
function E2_3_Page:methods()

end

---Listen 监控模块数仓变化，这里注册事件接口
function E2_3_Page:listen()
	return{
        cipher = {
            checkCipherResult = handler(self,self.showResult)
        }
	}
end

---showResult 将接收到的数据显示在页面上
function E2_3_Page:showResult(result,number)
    self.data.msgText = result
...
function E2_3_Page:observerDataStore()
    return {
        cipher = {
...
function E2_3_Page:onCliperPasswordChanged(oldVal,newVal)
...
function E2_3_Page:onCreated(options)
...
---@param text string
function E2_3_Page:onInputNameValueChanged(text)
...
-----OnClick_E_BtnClose()
function E2_3_Page:OnClick_E_BtnClose()
    self:closeMe()
end

-----OnClick_E_BtnSubmit()
function E2_3_Page:OnClick_E_BtnSubmit()
    Store.cipher:checkCipher(tonumber(self.data.inputPasswordText))
end

---BindCustomFunction绑定自定义事件方法
function E2_3_Page:BindCustomFunction()

end


--regionAutoBind 导出控件代码自动生成，请勿手动修改
function E2_3_Page:initAutoBind(gameObject)
    ---@type UIExpansion
...
function E2_3_Page:BindCustomModules()
end

---BindCustomFunction绑定通用事件方法
function E2_3_Page:BindCommonFunction()
