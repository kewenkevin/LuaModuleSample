...
module "LuaModules.Pbc.protobuf"
...
local function encode_message(CObj, message_type, t)
	local typeList = encode_type_cache[message_type]
	for k,v in pairs(t) do
		local func = typeList[k]
...
function encode( message, t , func , ...)
	...
	local errorMsg = encode_message(encoder, message, t)
	if errorMsg then
		return nil, errorMsg
	end
	if func then
...
function decode_message_mt.__call(tbl)
	expand(tbl)
