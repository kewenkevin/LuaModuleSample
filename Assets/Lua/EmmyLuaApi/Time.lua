---@class Time : Object
---@field public time number
---@field public timeAsDouble number
---@field public timeSinceLevelLoad number
---@field public timeSinceLevelLoadAsDouble number
---@field public deltaTime number
---@field public fixedTime number
---@field public fixedTimeAsDouble number
---@field public unscaledTime number
---@field public unscaledTimeAsDouble number
---@field public fixedUnscaledTime number
---@field public fixedUnscaledTimeAsDouble number
---@field public unscaledDeltaTime number
---@field public fixedUnscaledDeltaTime number
---@field public fixedDeltaTime number
---@field public maximumDeltaTime number
---@field public smoothDeltaTime number
---@field public maximumParticleDeltaTime number
---@field public timeScale number
---@field public frameCount number
---@field public renderedFrameCount number
---@field public realtimeSinceStartup number
---@field public realtimeSinceStartupAsDouble number
---@field public captureDeltaTime number
---@field public captureFramerate number
---@field public inFixedTimeStep bool
local Time={ }
UnityEngine.Time = Time