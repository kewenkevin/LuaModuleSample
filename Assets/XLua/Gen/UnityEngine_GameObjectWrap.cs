...
namespace XLua.CSObjectWrap
{
    ...
    public class UnityEngineGameObjectWrap 
    {
        ...
        static int __CreateInstanceNew(RealStatePtr L)
        {
			...
			try {
                ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
				if(LuaAPI.lua_gettop(L) == 1 && (LuaAPI.lua_isnil(L, 1) || LuaAPI.lua_type(L, 1) == LuaTypes.LUA_TSTRING))
				{
				...
				if(LuaAPI.lua_gettop(L) >= 1 && (LuaAPI.lua_isnil(L, 1) || LuaAPI.lua_type(L, 1) == LuaTypes.LUA_TSTRING) && (LuaTypes.LUA_TNONE == LuaAPI.lua_type(L, 2) || translator.Assignable<System.Type>(L, 2)))
				{
					string _name = LuaAPI.lua_tostring(L, 1);
					System.Type[] _components = translator.GetParams<System.Type>(L, 2);
